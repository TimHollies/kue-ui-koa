...
const koakue = () => {
  ...
  server.use(router.get('/job/search', async (ctx) => {
  ...
  for(const route of jsonApiGet) {
    server.use(router.get(route[0], async (ctx, ...args) => {
      ctx.body = await route[1](...args);
    }))
  }

  server.use(router.put('/job/:id/state/:state', async (ctx, ...args) => {
    ctx.body = await json.updateState(...args);
  }));

  server.use(router.put('/job/:id/priority/:priority', async (ctx, ...args) => {
    ctx.body = await json.updatePriority(...args);
  }));

  server.use(router.del('/job/:id', async (ctx, ...args) => {
    ctx.body = await json.remove(...args);
  }));

  server.use(router.post('/job', async (ctx, ...args) => {
    //use bodyparser
  ...
  for(const route of jobRoutes) {
    server.use(router.get(route[0], async (ctx) => {
